...
// Substitua a seção de conteúdo principal por esta versão corrigida:
    ...
    {slides.length > 0 ? slides.map((item, idx) => {
      ...
      return (
        <div key={idx} className="w-full flex-shrink-0 grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] h-full gap-0">
          {/* COLUNA ESQUERDA - Galeria, detalhes E tratamentos relacionados */}
          <div className="flex flex-col h-full justify-start px-4 sm:px-6 overflow-y-auto lg:overflow-hidden pb-20 lg:pb-0">
            {/* PARTE SUPERIOR: Galeria e informações do produto */}
            <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 xl:gap-8 w-full justify-center mt-4 lg:mt-0 lg:px-4 lg:pt-[5%]">
              ...
              <div className="flex flex-col gap-2 sm:gap-3 w-full lg:w-1/2 justify-start lg:justify-start mt-4 lg:mt-0">
            ...
            {/* PARTE INFERIOR: TRATAMENTOS RELACIONADOS - Na mesma coluna, embaixo */}
            {relatedTreatments.length > 0 && (
              <div className="mt-8 lg:px-4">
                {/* DESKTOP - Layout horizontal compacto */}
                <div className="hidden md:block">
                  {/* Título alinhado à esquerda, mesma margem que o conteúdo acima */}
                  <h2 className="text-xl font-bold text-black mb-4 text-left">Tratamentos Relacionados</h2>
                  
                  {/* Container com setas e produtos */}
                  <div className="flex items-center gap-3">
                    {/* Seta esquerda */}
                    <button
                      onClick={prevSlide}
                      disabled={slideIndex === 0}
                      className="bg-white/90 backdrop-blur rounded-full p-2 shadow-lg hover:bg-white transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
                    >
                    ...
                    {/* Grid de 4 itens mais compactos */}
                    <div className="grid grid-cols-4 gap-3 flex-1">
                      {relatedTreatments.slice(slideIndex, slideIndex + 4).map((relatedTreatment, index) => (
                        ...
                        >
                          <div className="w-full aspect-square bg-gray-100 rounded-lg mb-2 overflow-hidden shadow-sm">
